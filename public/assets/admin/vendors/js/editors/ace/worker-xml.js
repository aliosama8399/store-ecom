		...
		default:
			try{
				...
				if(!el.closed && fixSelfClosed(source,end,el.tagName,closeMap)){
					el.closed = true;
					if(!entityMap.nbsp){
						errorHandler.warning('unclosed xml attributes');
					}
				}
	...
	while(true){
		...
		case '=':
			if(s === S_ATTR){//attrName
				...
				throw new Error('attributes equal must after attrName');
			}
			break;
		case '\'':
		case '"':
			if(s === S_EQ){//equal
				...
				if(p>0){
					...
					throw new Error('attributes value no end \''+c+'\' match');
				}
			}else if(s == S_V){
				value = source.slice(start,p).replace(/&#?\w+;/g,entityReplacer);
				el.add(attrName,value,start);
				errorHandler.warning('attributes "'+attrName+'" missed start quot('+c+')!!');
				start = p+1;
				s = S_E
			}else{
				throw new Error('attributes value must after "="');
			}
			break;
		case '/':
			switch(s){
			...
			case S_ATTR_S:
				break;
			default:
				throw new Error("attributes invalid close char('/')")
			}
			break;
		case ''://end document
		...
		case '>':
			...
			case S_ATTR_S:
				...
				if(s == S_V){
					errorHandler.warning('attributes "'+value+'" missed quot(")!!');
					el.add(attrName,value.replace(/&#?\w+;/g,entityReplacer),start)
				}else{
					errorHandler.warning('attributes "'+value+'" missed value!! "'+value+'" instead!!')
					el.add(value,value,start)
				}
				break;
			case S_EQ:
				throw new Error('attributes value missed!!');
			}
			return p;
		case '\u0080':
			c = ' ';
		default:
			if(c<= ' '){//space
				...
				switch(s){
				case S_ATTR_S:
					errorHandler.warning('attributes "'+attrName+'" missed value!! "'+attrName+'" instead!!')
					el.add(attrName,attrName,start);
					start = p;
					s = S_ATTR;
					break;
				case S_E:
					errorHandler.warning('attributes space is required"'+attrName+'"!!')
				case S_S:
					s = S_ATTR;
					start = p;
					break;
...
function appendElement(el,domBuilder,parseStack){
	...
	while(i--){
		a = el[i];
		var prefix = a.prefix;
		if(prefix){//no prefix attributes has no namespace
			if(prefix === 'xml'){
...
ElementAttributes.prototype = {
	...
	add:function(qName,value,offset){
		if(!tagNamePattern.test(qName)){
			throw new Error('invalid attributes:'+qName)
		}
